...
#    define KBSTAT_VERSION 9   // 9: 慣性を撤去（b[24] は常に0、0xB6 は無くなった）
...
// トラックボールの慣性は撤去した（2026-09-13）
//
// 2026-08-13 に入れたが「割に合わない」と判断して既定ONのまま残していたものを、
// 使わないと決めたので消した。
//   ・トラックボールは既に物理的な慣性を持っている（指を離してもボールが空転し、
//     センサは 17 → … → 2 → 1 → 0 となだらかに減速した値を出し続ける）
//   ・「精密に止められる」というトラックボールの強みと正面から競合する
// 実測値と経緯は ClaudeCode/keyball/tools/README.md。戻すなら commit ee5a387 以前。
//
// これに伴い QK_USER_0（慣性のON/OFF）と Raw HID 0xB6（調整値）も無くなった。
// pointing_device_task_user / process_record_user も要らなくなったので置いていない。
// --------------------------------------------------------------------------
