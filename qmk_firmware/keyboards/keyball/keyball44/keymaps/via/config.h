...
// RETRO_TAPPING は使わない（2026-09-13 撤去）
//   ホールド判定されたあと結合キーを押さずに離すと、タップが送られてしまう。
//   ホームローmod（[⌥⌘]A / [⌘]F / [R⌃]H / [⇧]SPC …）で「修飾だけ押して離す」
//   たびに文字が出るのが実用上つらいため外した。
//   副作用: 英数/かな を TAPPING_TERM 超のゆっくりタップで押すと何も出ない。
//   再発したら RETRO_TAPPING を戻すのではなく、TAPPING_TERM_PER_KEY で
//   タップ側が LNG1/LNG2 のキーだけ判定時間を延ばすこと（他キーに副作用が出ない）。
